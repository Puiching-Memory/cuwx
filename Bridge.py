...
def main(module_name):
    ...
    class Main(GUI_module.Main):
        def __init__(self, parent):
            ...
            sizer_line, sizer_def, sizer_do, sizer_name, sizer_structure = self.Analyze(
                path
            )
        ...
        def list_instance_variables(self):
            ...
            instance_variables = [
                ...
                if not callable(getattr(type(self), attr, None))
            ]
            instance_variables = [
                attr for attr in instance_variables if not attr.startswith("__")
            ]
            instance_variables = [
                attr for attr in instance_variables if not hasattr(type(self), attr)
            ]
            ...
            for i in sizer_do:
                ...
                for ia in sizer_name:
                    if ia in i:
                        ...
                        info = i
                        info = (
                            info[info.find("Add") + 5 : -2].replace(" ", "").split(",")
                        )
                        ##print(info)
                        target.append(info)
                        break
